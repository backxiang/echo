...
<% if session("userid")=cint(usid) then%>
     <li class="active" data-toggle="tab"><a href="moments.asp" >好友动态</a></li>
...
conn.Open
sqlfri="select * from friend where userid='"&session("userid")&"'"
set rsfri=conn.execute(sqlfri)


' 如果还没有好友
if rsfri.EOF then
response.write"<div class='item'> 您还没有好友噢~ </div>"
else
end if
' sql10="select * from comment"
' 如果有好友则循环好友
 while not rsfri.eof
   sqlfollow="select * from comment where userid='" &rsfri("friendid")&"' order by commenttime desc "
   set rsfollow=conn.execute(sqlfollow)
' 查询好友的名字
   sqlf="select * from users where userid=" &rsfri("friendid")
...
' 循环单个好友的评论
       ...
       <%
         ...
         id=rsfollow("movieid")
         set rs1=conn.execute("select * from movie where movieid='"& id &"'")%>

         <div class="item">
                 <div class="author"></div>
                 <div class="status"><a href="profile.asp?usid=<%=rsf("userid")%>"><%=rsf("username")%></a>&nbsp评论于：<%=rsfollow("commenttime")%></div>
                 ...
                 <div class="words"><%=rsfollow("content")%></div>
                 <div class="status" align="right">赞&nbsp<%=rsfollow("likes")%></div>
         </div>
...
<a href="myfavorites.asp?ussid=<%=rst("userid")%>">
  <% if session("userid")=usid then%>   <h2>我的收藏夹</h2></a>
