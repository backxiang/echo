...
<head>
  ...
  <%
    ...
    usid=request("usid")
    if session("userid")="" then usid=request("usid")
...
<!-- 关注和取消关注按钮 -->
   ...
   if rsg.eof then  %>
 <form method="POST" action="addfriend.asp?ussid=<%=rst("userid")%>" name="form2">
 <input class="btn btn-success" align="right"type="submit" value="关注ta"></input>
</form>
   <%else%>
   <form method="POST" action="delfriend.asp?ussid=<%=rst("userid")%>" name="form2">
 <input class="btn btn-success" align="right"type="submit" value="取消关注"></input>
</form>
   <%end if%>
<%end if%>
...
<% if session("userid")=cint(usid) then%>
     ...
     <li class="active"><a href="profile.asp" data-toggle="tab">我的影评</a></li>
<%else%><li class="active" data-toggle="tab"><a href="profile.asp?usid=<%=rst("userid")%>"><%=rst("username")%>的影评</a></li>
<%end if%>
<li><a href="myrate.asp?usid=<%=rst("userid")%>">评过分的电影</a></li>
<li><a href="mylikes.asp?usid=<%=rst("userid")%>">点过赞的评论</a></li>
...
<%
set rs= cnn.execute("select * from comment where userid='" & usid &"' order by commenttime desc")
if rs.eof then
response.write "<div class='item'> 当前暂无评论~ </div>"
%>
<%end if%>

<% While Not rs.eof
id=rs("movieid")
set rs1=cnn.execute("select * from movie where movieid='"& id &"'" )
set rs5=cnn.execute("select * from users where userid="&usid )%>
<div class="item">
        <div class="status">
          &nbsp&nbsp评论于：<%=rs("commenttime")%></div>
        <a style="position:relative;display:block;"  href="detail.asp?movid=<% =rs1("MovieId") %>" target="view_window">        <div class="moviename"><%=rs1("moviename")%></div></a>
        <div class="words"><%=rs("content")%></div>
        <div class="status" align="right"style="padding-right: 40px;">赞&nbsp<%=rs("likes")%></div>
</div>
<%
n=n+1
rs.movenext
