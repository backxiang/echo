...
<li class="active"><a href="myboard.asp?usid=<%=rst("userid")%>">留言板</a></li>
  </ul>
</div>
<br>
...
<!-- 查询留言板信息 -->
<%set rs= cnn.execute("select * from board where boardid='" & usid &"' ")
' 如果没有留言
if rs.eof then
response.write "<div class='item'> 这里还没有留言噢 </div>"
end if%>

<!-- 如果有留言 -->
<%while not rs.eof%>
      <%set rs1=cnn.execute("select * from board where userid='"&rs("userid")&"'")
      if rs1.eof then response.write""%>

      <%While Not rs1.eof%>
      <%set rs5=cnn.execute("select * from users where userid="&rs1("userid"))%>
      <!-- 循环留言信息 -->
     <div class="item">
       <a href="profile.asp?usid=<% =rs1("userId") %>" target="view_window"><img class="cont-pic" width="30" height="30" src="avatars/<%=rs5("avatar")%>"></a>
       <div class="" style="padding-top:5px;"><%=rs5("username")%></div>
       <div class="message">留言内容:&nbsp&nbsp<%=rs1("message")%></div>
       <div class="messagetime">留言时间:&nbsp&nbsp<%=rs1("messagetime")%></div>
     </div>
...
<div class="comment" align="center" >
  <form method="POST" action="newmessagesave.asp?boardid="&usid name="formadd" onsubmit = "return ChkFields()">
    <div class="wrapper" align="left">
    <h3 style="margin-top: 10px;">我来说两句</h3>
    </div>
    <div class="words">
      <textarea rows="5" cols="90" name="message" class="texts" ></textarea>
    </div>
