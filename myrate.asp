...
<%end if%>
<li class="active"><a href="myrate.asp?usid=<%=rst("userid")%>">评过分的电影</a></li>
...
<%
set rs= cnn.execute("select * from grade where userid='" & usid &"' ")
if rs.eof then
response.write "<div class='item'> 尚未对电影评分 </div>"
%>
<%end if%>
...
<div class="item">
  <div class="row">
      <div class="col-md-3 " align="center">
        <a href="detail.asp?movid=<% =rs1("MovieId") %>" target="view_window"><img class="cont-pic" width="130" height="180" src="posters/<%=rs1("MovieId")%>.jpg"></a>
        <div class="status" style="padding-top:5px;"><%=rs5("username")%>的评分<%=rs("score")%>分</div>
      </div>
      <div class="col-md-9">

        <a style="position:relative;display:inline-block;"  href="detail.asp?movid=<% =rs1("MovieId") %>" target="view_window">
        <div class="moviename"><%=rs1("moviename")%></div></a>
        <div class="rating" style="padding-top:1px;display:inline-block;"><%=rs1("score")%>分</div>
<div class="con">
  <div class="rate">导演:&nbsp&nbsp<%=rs1("director")%></div>
  <div class="rate">演员:&nbsp&nbsp<%=rs1("cast")%></div>
  <div class="rate">上映年份:&nbsp&nbsp<%=rs1("ReleaseDate")%></div>
  <div class="rate">电影类型:&nbsp&nbsp<%=rs1("type")%></div>
</div>
</div>
  </div>
</div>
